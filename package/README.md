...
- **macOS ARM**：本机执行 `./packaging/build_dmg.sh` 后，将 `dist/Media Browser v1.2.1.dmg` 复制到本目录。
- **Windows**：在 Windows 上执行 `packaging/build_windows.ps1`，或推送标签 `v1.2.1` 后从 GitHub Actions `release.yml` 的 **windows** artifact 下载 `MediaBrowser-v1.2.1-windows.zip` 复制到本目录。
...
```bash
docker build -t media-browser:1.2.1 .
docker save media-browser:1.2.1 | gzip -1 > package/media-browser-1.2.1-linux-$(uname -m).tar.gz
```
