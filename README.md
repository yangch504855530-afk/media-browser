...
| 按键 | 视频 | 图片 |
|------|------|------|
| `←` / `→` | 快退 / 快进 **5 秒** | 上一张 / 下一张（**末张再按 →** 进入**下一作品首张**；首张再按 ← 进入**上一作品末张**） |
| `Shift + ←` / `Shift + →` | 上一个 / 下一个文件（**末档再按 →** 进入**下一作品首张**；反之进入**上一作品末张**） | — |
| `J` / `L` | 快退 / 快进 **10 秒** | — |
| `空格` / `K` | 播放 / 暂停 | — |
| `F` | 视频**全屏** | — |
| `M` | **静音**切换 | — |
...
画廊内 **`⌘←` / `⌘→`** 用于**切换作品**；普通 **`←` / `→`** 仍为快进/快退（视频）或上一张/下一张（图片），互不冲突。

### 审阅列表快捷键（画廊关闭、焦点不在输入框内）
...
与上表相同按键：列表页为**滚动定位卡片**；画廊打开时为**打开相邻作品**（见上表）。
...
`mb_search`、`mb_sort`、`mb_filter` 等 UI 偏好仍存浏览器本地。

**审阅进度（v1.4.0+）**：作品级「保留/待审」与断点位置存于 NAS **`cache/review/`** 审阅账本，跨手机/电脑自动同步；切换 `MB_SCAN_PRESETS` 媒体库时各用独立账本。旧版浏览器里的 `mb_review_tags` 会在首次加载时一次性导入服务端。
