...
**当前版本**以 `media_browser.py` 中 `APP_VERSION` 与页眉 `v…` 为准（与本文同步时常为 **1.0.3** 及以上，发版前请与打包配置核对）。
...
**尚未替代的需求**：若需 **macOS 原生「浏览文件夹」** 选根目录（而非手写/粘贴路径），仍可作为后续增强。

### 仍可排的 backlog（约定后再做）

**P1 体验与安全**：删除前可选二次确认；删除进废纸篓；转码更细进度；默认仅监听 `127.0.0.1` 等。

**P2 功能**：搜索增强、审阅标签导出/导入、非 macOS 打开目录、窄屏侧栏。
